...
* Visual FoxPro 7.0 MENU BUILDER.
*
* Modification History:
* December 13, 1990		JAC		Program Created
*
* Modified for FoxPro 2.5 by WJK
* Modified for Visual FoxPro 3.0 by DTA
* Modified for Visual FoxPro 5.0-7.0 by RB
*
******************************************************************************
...
SCAN FOR objtype=c_item AND LOWER(levelname)=m.popname
   IF INLIST(LOWER(ALLTRIM(name)),"_mwz_setup","_mtl_spell","_mti_runactivedoc")
      LOOP
   ENDIF
...
*
* ADDPICTURE - Generate PICTURE... clause whenever appropriate.
*
* Description:
* Adds a PICTURE/PICTRES clause for the DEFINE BAR command
*
FUNCTION addpicture
LOCAL lcPictName,lcPictType, lcFileName
IF FCOUNT() < c_70mnxflds
	RETURN
ENDIF
lcPictName = ALLTRIM(resname)
lcPictType = sysres
IF m.lcPictType#1 AND !EMPTY(m.lcPictName) AND g_imagepaths
	m.lcFileName = FULLPATH(m.lcPictName,g_mnxfile[1])
	IF FILE(m.lcFileName)
		m.lcPictName = m.lcFileName
	ENDIF
ENDIF
IF NOT EMPTY(m.lcPictName)
   \\ ;
   IF m.lcPictType=1
	   \	PICTRES <<m.lcPictName>>
   ELSE
	   \	PICTURE "<<m.lcPictName>>"
   ENDIF 
ENDIF
...
* clause.
*
* VFP7 - we now ignore if SKIP .F. and pass remaining contents
*
FUNCTION addskipfor
PRIVATE m.skip
...
FOR m.i = 1 TO m.lines
   ...
   CASE LEFT(UPPER(LTRIM(m.thisline)),6) == "#IMAGE"   && #IMAGEPATHS
      m.g_imagepaths=.T.
 ...
 * 
 * *****************  Version 15  *****************
 * User: rb          Date: 10/04/01
 * Updated in $/Genmenu
 *
 * Improved SKIP FOR .F. handling
 * *****************  Version 14  *****************
 * User: rb          Date: 11/07/00 
 * Updated in $/Genmenu
 *
 * Added code to skip obsolete menu bars:
 *   Spell Checker, RunActiveDoc, Setup Wizard
 *
 * New #IMAGEPATHS directive (place in Setup snippet) 
 * to generate fullpaths for images.
 *
 * *****************  Version 13  *****************
 * User: rb          Date: 09/01/00 
 * Updated in $/Genmenu
 *
 * We now avoid emitting SKIP FOR .F. and just emit 
 * content after .F.
 *
 * *****************  Version 12  *****************
 * User: rb          Date: 07/01/99    Time: 1:04
 * Updated in $/Genmenu
 *
 * Added support for new DEFINE BAR ... PICTURE/PICTRES
 * 
 * *****************  Version 11  *****************
